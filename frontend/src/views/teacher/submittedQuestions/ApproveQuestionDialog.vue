<template>
    ...
    >
        <v-card>
            <v-card-title>
                <span class="headline">
                  Why did you approve this question?
                </span>
            </v-card-title>
            ...
            <v-card-actions>
                ...
                <v-btn color="blue darken-1" @click="saveQuestion"
                       data-cy="closeJustificationApproved"
                >Approve Question</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
...
<script lang="ts">
    ...
    @Component
    export default class ApproveQuestionDialog extends Vue {
        ...
        async saveQuestion() {
            if (
            ...
            else if (this.editQuestion && this.editQuestion.state === 'APPROVED') {
                await this.$store.dispatch(
                    'error',
                    'Question was already approved'
                );
                return;
            }
                ...
                try {
                    await RemoteServices.setQuestionState(this.editQuestion.id, 'APPROVED');
