...
public class StatsService {
    ...
    public StatsDto getStats(String username, int executionId) {
        ...
        int totalSubmittedQuestions = questionRepository.getSubmittedQuestionsStudent(course.getId(), user.getUsername());
        int totalApprovedQuestions = questionRepository.getApprovedQuestionsStudent(course.getId(), user.getUsername());
        int totalDisapprovedQuestions = questionRepository.getDisapprovedQuestionsStudent(course.getId(), user.getUsername());

        boolean dashboard = user.getDashboard();

        int numberOfClarifications = clarificationRepository.getNumberOfRequests(username);
        int numberOfResponses = clarificationRepository.getNumberOfResponses(username);
        ...
        statsDto.setTotalSubmittedQuestions(totalSubmittedQuestions);
        statsDto.setTotalApprovedQuestions(totalApprovedQuestions);
        statsDto.setTotalDisapprovedQuestions(totalDisapprovedQuestions);
        statsDto.setDashboard(dashboard);

        statsDto.setNumberOfClarifications(numberOfClarifications);
        statsDto.setNumberOfResponses(numberOfResponses);
    ...
    @Retryable(
            ...
            backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public void setDashboard(String username, boolean dashboard){
        User user = userRepository.findByUsername(username);
