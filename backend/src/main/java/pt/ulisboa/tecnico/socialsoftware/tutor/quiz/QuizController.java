...
public class QuizController {
    ...
    @PostMapping("/executions/tournament/quizzes")
    public QuizDto createTournamentQuiz(@Valid @RequestBody Tournament tournament) {
        return this.quizService.createTournamenteQuiz(tournament);
    }
    ...
    private void formatDates(QuizDto quiz) {
        ...
        if (quiz.getAvailableDate() != null
                && !quiz.getAvailableDate().matches("(\\d{4})-(\\d{2})-(\\d{2}) (\\d{2}):(\\d{2})")) {
            quiz.setAvailableDate(
                    LocalDateTime.parse(quiz.getAvailableDate().replaceAll(".$", ""), DateTimeFormatter.ISO_DATE_TIME)
                            .format(formatter));
        }
        if (quiz.getConclusionDate() != null
                && !quiz.getConclusionDate().matches("(\\d{4})-(\\d{2})-(\\d{2}) (\\d{2}):(\\d{2})"))
            quiz.setConclusionDate(
                    LocalDateTime.parse(quiz.getConclusionDate().replaceAll(".$", ""), DateTimeFormatter.ISO_DATE_TIME)
                            .format(formatter));
    }
}