...
import pt.ulisboa.tecnico.socialsoftware.tutor.quiz.domain.Quiz.QuizType;
...
public class QuizService {
    ...
    public CourseDto findQuizCourseExecution(int quizId) {
        return this.quizRepository.findById(quizId).map(Quiz::getCourseExecution).map(CourseDto::new)
    ...
    public List<QuizDto> findNonGeneratedQuizzes(int executionId) {
        ...
        return quizRepository.findQuizzes(executionId).stream()
                .filter(quiz -> !quiz.getType().equals(Quiz.QuizType.GENERATED)).sorted(comparator)
                .map(quiz -> new QuizDto(quiz, false)).collect(Collectors.toList());
    }
    ...
    @Retryable(value = { SQLException.class }, backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public QuizDto createTournamenteQuiz(Tournament tournament) {

        Quiz quizzTournament = new Quiz();
        quizzTournament.setKey(getMaxQuizKey() + 1);
        CourseExecution courseExecution = courseExecutionRepository.findById(tournament.getCourseExecutionId())
                .orElseThrow(() -> new TutorException(COURSE_EXECUTION_NOT_FOUND, tournament.getCourseExecutionId()));
        quizzTournament.setCourseExecution(courseExecution);

        User creator = userRepository.findById(tournament.getCreatorId())
                .orElseThrow(() -> new TutorException(USERNAME_NOT_FOUND, tournament.getCreatorId()));

        String userName = creator.getName();
        quizzTournament.setTitle("Tournament created by " + userName);

        quizzTournament.setType(QuizType.PROPOSED);
        quizzTournament.setCreationDate(LocalDateTime.now());
        quizzTournament.setAvailableDate(tournament.getStartTime());
        quizzTournament.setConclusionDate(tournament.getEndTime());

        final List<Integer> questionsList = questionRepository.findByTopic(tournament.getTopicId());

        Collections.shuffle(questionsList);

        for (int i = 0; i < tournament.getNumberQuestions(); i++) {
            int currentQuestionId = questionsList.get(i);
            Question question = questionRepository.findById(currentQuestionId)
                    .orElseThrow(() -> new TutorException(QUESTION_NOT_FOUND, currentQuestionId));

            QuizQuestion quizQuestion = new QuizQuestion(quizzTournament, question,
                    quizzTournament.getQuizQuestions().size());

            quizQuestionRepository.save(quizQuestion);

        }

        quizRepository.save(quizzTournament);

        return new QuizDto(quizzTournament, true);
    }
    ...
    public QuizAnswersDto getQuizAnswers(Integer quizId) {
        ...
        quizAnswersDto.setCorrectSequence(
                quiz.getQuizQuestions().stream().sorted(Comparator.comparing(QuizQuestion::getSequence))
                        .map(quizQuestion -> quizQuestion.getQuestion().getOptions().stream().filter(Option::getCorrect)
                                .findFirst().orElseThrow(() -> new TutorException(NO_CORRECT_OPTION)).getSequence())
        ...
        if (quiz.getConclusionDate() != null && quiz.getConclusionDate().isAfter(LocalDateTime.now())) {
            quizAnswersDto
                    .setSecondsToSubmission(ChronoUnit.SECONDS.between(LocalDateTime.now(), quiz.getConclusionDate()));
        }
    ...
    public void resetDemoQuizzes() {
        quizRepository.findQuizzes(Demo.COURSE_EXECUTION_ID).stream().filter(quiz -> quiz.getId() > 5360)
                .forEach(quiz -> {
                    for (QuizAnswer quizAnswer : new ArrayList<>(quiz.getQuizAnswers())) {
                        answerService.deleteQuizAnswer(quizAnswer);
                    }

                    for (QuizQuestion quizQuestion : quiz.getQuizQuestions().stream()
        ...
        for (Question question : questionRepository.findQuestions(Demo.COURSE_ID).stream()
                .filter(question -> question.getQuizQuestions().isEmpty()).collect(Collectors.toList())) {
